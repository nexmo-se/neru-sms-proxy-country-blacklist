...
if (!process.env.API_ACCOUNT_ID) {
  console.log('Missing API_ACCOUNT_ID', process.env.API_ACCOUNT_ID);
}

var session = neru.getSessionById(process.env.API_ACCOUNT_ID); // zzzbeejay
...
app.use(
  ...
  [countryblacklist.blacklist_to, countryblacklist.number_to],
...
app.get('/blacklist', async (req, res, next) => {
  ...
  res.status(200).send(JSON.parse(blacklist));
});
